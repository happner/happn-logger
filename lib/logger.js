...
module.exports.configure = function(config) {
  ...
  if (!Config.logger) {
    ...
    Config.logger = {
      appenders: {
        console:{ type: "console", layout: Config.logLayout}
      },
      categories: {
  ...
  if (Config.logFile && defaulting) {
    Config.logger.appenders.fileAppender = {
      type: "file",
    ...
    Config.logger.categories.default.appenders.push('fileAppender');

    if (Config.logFileLayout) {
      Config.logger.appenders.fileAppender.layout = Config.logFileLayout
    } else {
      Config.logger.appenders.fileAppender.layout = {
        type: 'pattern',
        pattern: '%d{' + (Config.logDateFormat || 'yyyy-MM-dd hh:mm:ss.SSS') + '} [%5.5p] - %m'
      }
    }
  ...
  Config.logger.categories.default.level = (Config.logLevel);
