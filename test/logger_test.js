...
describe('Logger', function() {
  ...
  context('configuration', function() {
    ...
    it('defaults config', function() {
      ...
      console.log("LOGGER CONFIG:::", JSON.stringify(Logger.config,
        null, 2));
  ...
  context('log functions', function() {
    ...
    it('defines a function to emit a log message at each level',
      function() {
        Logger.configure();
    ...
    it('formats the log text from multiple arguments', function(done) {
      Logger.configure();
      Logger.config.log = function(level, context, component,
        message) {
    ...
    it(
      'includes the array of log arguments in log call if logStackTraces is enabled',
      function(done) {
        Logger.configure({
          logStackTraces: true,
        });
  ...
  context('logging to file', function() {
    ...
    it(
      'can includes a stack trace if enabled and the last arg is an error',
      function(done) {
        try {
          fs.unlinkSync('file.log');
        } catch (e) {}
  ...
  context('component filter', function() {
    ...
    it('logs fatal, error and warn from even if filtered out',
      function(
        done) {
        try {
          fs.unlinkSync('file.log');
  ...
  context('logger subconfig', function() {
    ...
    it('allows for entirely externally defined log4js config', function(
      done) {
  ...
  context('logger after event', function() {
    ...
    it(
      'allows for a listener to be inserted, checks the listener receives all input',
      function(done) {
        ...
        Logger.emitter.on('after', function(level, message) {
          ...
          if (Object.keys(listened).length == 3) {
            ...
            listened['info'].split('\t')[1].should.equal(
              'INFO-TEST');
            listened['warn'].split('\t')[1].should.equal(
              'WARN-TEST');
            listened['error'].split('\t')[1].should.equal(
              'ERROR-TEST');
